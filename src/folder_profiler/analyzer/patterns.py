...
class PatternDetector:
    ...
    def detect_patterns(self, folder_tree: FolderNode) -> Dict[str, Any]:
        ...
        return {
            ...
            "build_artifacts": self._format_file_list(self.detect_build_artifacts(all_files)),
    ...
    def detect_version_patterns(self, files: List[FileInfo]) -> List[Dict[str, Any]]:
        ...
        version_patterns = [
            (r"(.+?)[-_\.]v?(\d+)\.(\d+)\.(\d+)", "semantic"),  # file-v1.2.3 or file_1.2.3
        ...
        for file in files:
            name_without_ext = file.name.rsplit(".", 1)[0] if "." in file.name else file.name
    ...
    def detect_build_artifacts(self, files: List[FileInfo]) -> List[FileInfo]:
        ...
        for file_info in files:
            # Check extension
            if file_info.extension and file_info.extension.lower() in artifact_extensions:
    ...
    def detect_naming_conventions(self, files: List[FileInfo]) -> Dict[str, int]:
        ...
        for file in files:
            ...
            elif re.match(r"^[a-z][a-zA-Z0-9]*$", name_without_ext) and any(c.isupper() for c in name_without_ext):
    ...
    def _format_file_list(self, files: List[FileInfo], limit: int = 50) -> List[Dict[str, Any]]:
